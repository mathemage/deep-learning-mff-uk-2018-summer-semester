...
class Network:
		...
		def construct(self, args):
				with self.session.graph.as_default():
						...
						hidden_layers = [None] * (1 + depth_of_hidden_layers)
						hidden_layers[0] = input_layer
						for depth in range(1, 1 + depth_of_hidden_layers):
							hidden_layers[depth] = tf.layers.dense(hidden_layers[depth - 1], width_of_hidden_layer,
							                                       activation=activation_function, name="hidden_layer{}".format(depth))
						last_hidden_layer = hidden_layers[-1]
						output_layer = tf.layers.dense(last_hidden_layer, self.ACTIONS, activation=None, name="output_layer")
