	...
	def construct(self, args, source_chars, target_chars, bow, eow):
		with self.session.graph.as_default():
			...
			class DecoderTraining(tf.contrib.seq2seq.Decoder):
				@property
				def batch_size(self):
					return tf.shape(source_states)[0]  # TODO: Return size of the batch, using for example source_states size
				...
				def initialize(self, name=None):
					finished = 0 >= target_lens  # TODO: False if target_lens > 0, True otherwise
				...
				def step(self, time, inputs, states, name=None):
					outputs, states = decoder_cell(inputs, states)  # TODO: Run the decoder GRU cell using inputs and states.
					outputs = decoder_layer(outputs)  # TODO: Apply the decoder_layer on outputs.
					# TODO: Next input are character embeddings with index `time` in target_embedded.
					next_input = target_embedded[:, time]
					finished = time + 1 >= target_lens  # TODO: False if target_lens > time + 1, True otherwise.
